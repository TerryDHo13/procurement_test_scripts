...
test.use({ storageState: 'states/.auth/procurementOfficer.json' });
...
test("New Initial Request Form, Valid Test", async ({ page }) => {
  ...
  await page.getByLabel('Title').fill('Request Form from Finance Officer');
  ...
  await page.getByLabel('Reason of Purchase').fill('This is entered by Finance Officer');
...
test('View Quotation Summary, Valid Test', async ({ page }) => {
  ...
  await page.getByLabel('IRF No.').click();
  await page.getByPlaceholder('Filter By IRF No.').press('CapsLock');
  await page.getByPlaceholder('Filter By IRF No.').fill('DHIRF/DH/2024/00043');
  await page.getByRole('button', { name: '󰍉 filter' }).click();
  await expect(page.getByRole('table')).toContainText('DHIRF/DH/2024/00043');
  await page.locator('div:nth-child(5) > .pl-1 > .v-btn').click();
  await expect(page.getByRole('main')).toContainText('IRF No.');

  await page.getByLabel('Title', { exact: true }).click();
  await page.getByPlaceholder('Filter By IRF Title').fill('Testing Add Quotation');
  await page.getByRole('button', { name: '󰍉 filter' }).click();
  await expect(page.getByRole('table')).toContainText('Testing Add Quotation');
  await page.locator('div:nth-child(6) > .pl-1 > .v-btn').click();
  await expect(page.getByRole('main')).toContainText('Title');
